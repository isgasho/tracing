...
// 	if err := storage.session.Query(util.AgentInsert,
// 		agentInfo.AppName,
// 		agentInfo.AgentID,
...
// 		agentInfo.OperatingEnv).Exec(); err != nil {
...
// // AgentInfoStore ...
// func (storage *Storage) AgentInfoStore(appName, agentID string, agentInfo []byte) error {
// 	// query := fmt.Sprintf(" `UPDATE agents SET agent_info='%s' WHERE app_name='%s' and agent_id='%s';`", appName, agentID, agentInfo)
// 	if err := storage.session.Query(util.AgentInfoInsert, appName, agentID, agentInfo).Exec(); err != nil {
// 		// if err := storage.session.Query(query).Exec(); err != nil {
// 		g.L.Warn("agentStore:storage.session.Query", zap.String("error", err.Error()), zap.String("sql", util.AgentInfoInsert))
// 		return err
// 	}
// 	return nil
